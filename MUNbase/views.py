...
def showallmotions(request,munname,commname):
    if request.is_ajax and request.method == "GET":
        ...
        length = len(Participant.objects.filter(committee=comm).exclude(status =''))
...
def getvoterdata(request, munname,commname, motionid):
    if request.is_ajax and request.method == "GET":
        ...
        try:
            ...
            return 'Motion Error'
        voters = Voter.objects.filter(motion = Motion.objects.filter(committee =comm, id = motionid)[0]).values()
        voterz = Voter.objects.filter(motion = Motion.objects.filter(committee =comm, id = motionid)[0])
...
def updatepwork(request, munname, commname):
    ...
    pwork = Paperwork.objects.filter(committee=comm, id = response['id'])
    try:
        pwork = pwork[0]
    except IndexError:
        return 'error'
...
def submitam(request, munname, commname, pid, type, cloz, content):
    if request.is_ajax and request.method == "GET":
        ...
        amm.save()
        return JsonResponse({'resps':'success'})
def pdfpwork(request, munname, commname, pid):
        if request.is_ajax and request.method == "GET":
            if request.session.get('emunalloc') is None or request.session.get('emuncomm') is None or request.session.get('emun') is None:
                return None
            munnamez = request.session.get('emun')
            ...
            try:
                ...
                return 'Comm Error'
            pp = Paperwork.objects.filter(committee=comm, id =pid)[0]
            return render(request, 'munfts/mymun/emun/ppdf.html', {'pwork':pp.body})
